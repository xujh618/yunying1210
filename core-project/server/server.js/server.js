...
async function insertDigitalLibraryDepartmentVisits(data) {
  ...
  try {
    ...
    if (tableCheckError && tableCheckError.code === '42P01') {
      // 表不存在，输出错误信息和手动创建表的SQL语句
      console.error('数字智库部门访问数据表不存在');
      console.log('请手动在Supabase后台执行以下SQL语句创建表:');
      console.log(`
            ...
            CREATE INDEX IF NOT EXISTS idx_department_visits_month ON digital_library_department_visits(month);
            CREATE INDEX IF NOT EXISTS idx_department_visits_name ON digital_library_department_visits(department_name);
          `);
      return inserted;
    }
  } catch (error) {
  ...
  // 判断数据格式
  // 1. 如果第一列看起来像月份（包含"月"字或者是日期格式），则按行存储格式处理
  // 2. 如果第一列看起来像部门名称，则按列存储格式处理
  const isMonthFirst = headers[0] && (headers[0].includes('月') || headers[0].includes('month'));
  
  if (isMonthFirst) {
    // 行存储格式：每行包含月份、部门名称和访问次数
    console.log('按行存储格式处理部门访问数据');
      ...
      if (row.length >= 3) {
        ...
        if (month && department_name) {
          ...
          if (!error) {
            ...
            console.error(`插入部门 ${department_name} 数据失败:`, error.message);
          }
        }
      }
    }
  } else {
    // 列存储格式：第一列是部门名称，后续各列是不同月份的访问次数
    console.log('按列存储格式处理部门访问数据');
    
    for (const row of rows) {
      if (row.length >= 2) {
        const department_name = row[0] || '';
  ...
  console.log('数字智库部门访问分布表插入:', inserted, '条数据');
  return inserted;
}
