...
async function insertDigitalLibraryDepartmentVisits(data) {
  ...
  // 根据用户描述，Excel数据是行存储格式：
  // 第一列是月份，第二列是部门名称，第三列是访问次数
  // 例如：2025-01 | 财务部 | 10
  
  // 判断数据格式：通过检查第一行数据来识别格式
  const firstRowFirstCol = rows[0] ? rows[0][0] : '';
  const firstRowSecondCol = rows[0] ? rows[0][1] : '';
  
  // 如果第一列看起来像月份（包含"-"或者是数字开头）或者第二列看起来像部门（包含"部"），则使用行存储格式
  const isRowFormat = firstRowFirstCol && (
    firstRowFirstCol.includes('-') || 
    /^\d{4}-\d{2}$/.test(firstRowFirstCol) ||
    firstRowSecondCol.includes('部')
  );
  
  console.log(`检测到${isRowFormat ? '行' : '列'}存储格式`);
